...
This release candidate focuses on Axion's in-game editing experience. It replaces bulky tool text with compact contextual hints, adds new mouse and tool-swapper GUI assets, improves scroll/input behavior around menus, and adds Iris-aware preview handling so shader packs no longer break Axion's large preview path.
...
- Added new mouse hint textures:
  - `assets/axion/mouse/left.png`
  - `assets/axion/mouse/right.png`
  - `assets/axion/mouse/scroll.png`
  - `assets/axion/mouse/neutral.png`
- Added a texture-backed tool swapper using `assets/axion/gui/tool_swapper.png`
- Replaced the old abbreviated `Mv`, `Cl`, `Sy`, etc. tool strip with icon-based slots
- Reverted the code-drawn hotbar-style slot for the Axion tool area and now uses the tool swapper atlas instead
...
- Improved Alt tool-swapper interaction so hovering and selecting tools matches the texture-backed slot layout
