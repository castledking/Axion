...
class AxionCapabilityPolicyTest {
    ...
    @Test
    fun `only force place ignores support requirements`() {
        // One switch covers every attachment rule in the game, so lanterns,
        // redstone, torches and rails all come along without being named.
        assertTrue(AxionCapabilityPolicy.ignoresSupportRequirements(GlobalModeState(forcePlaceEnabled = true)))
        assertFalse(AxionCapabilityPolicy.ignoresSupportRequirements(GlobalModeState(noUpdatesEnabled = true)))
        assertFalse(AxionCapabilityPolicy.ignoresSupportRequirements(GlobalModeState()))
    }
