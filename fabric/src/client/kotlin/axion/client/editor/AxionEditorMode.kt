...
object AxionEditorMode {
    ...
    private fun speedMultiplier(): Float =
        FLIGHT_SPEED_MULTIPLIER * AxionEditorUiBridge.flightSpeedPercent / 100f
    ...
    fun onMouseButton(client: MinecraftClient, button: Int, action: Int): Boolean {
        ...
        when (action) {
            GLFW.GLFW_PRESS -> {
                val uiConsumed = AxionEditorUiBridge.onMouseButton(button = button, pressed = true)
            ...
            GLFW.GLFW_RELEASE -> {
                AxionEditorUiBridge.onMouseButton(button = button, pressed = false)
    ...
    fun toggle(client: MinecraftClient) {
        ...
        if (!isEnabled) {
            ...
            if (!AxionEditorUiBridge.isAvailable()) {
                // owo-lib is optional; without it there are no panels to show,
                // so the editor stays off rather than half-working.
                showStateMessage(client, "requires_owo")
                return
            }
