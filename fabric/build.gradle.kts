...
val verifyIntegratedNoClipWiring by tasks.registering {
    ...
    doLast {
        ...
        // Force Place leans on a single support-check hook rather than a block
        // list, so losing it silently reverts the capability to vanilla rules.
        check("BlockStateSupportMixin" in clientMixins) {
            "Force Place support mixin is not enabled for Minecraft $minecraftVersion: $mixinConfig"
        }
        check(clientClasses.resolve("axion/mixin/client/BlockStateSupportMixin.class").isFile) {
            "Force Place support mixin was not compiled for Minecraft $minecraftVersion"
        }
...
val verifyPreviewVisualCoverage by tasks.registering {
    ...
    doLast {
        ...
        val previewFragmentShader = file(
            "src/client/resources/assets/axion/shaders/core/preview_shell.fsh",
        ).readText()
        check(
...
val verifyFabricServerRangeCompatibility by tasks.registering {
    ...
    doLast {
        val networkingSource = rootProject.file(
...
val verifyAuthoritativeHistoryReplay by tasks.registering {
    ...
    doLast {
        listOf(
            rootProject.file("paper-plugin/src/main/kotlin/axion/server/paper/AxionHistoryActionService.kt"),
            rootProject.file("fabric-server/src/main/kotlin/axion/server/fabric/AxionFabricHistoryActionService.kt"),
        ).forEach { serviceFile ->
            val source = serviceFile.readText()
            val servicePath = serviceFile.path
        ...
        val transport = rootProject.file(
        ...
        check(
            "max-bytes: 268435456" in rootProject.file("paper-plugin/src/main/resources/config.yml").readText() &&
                "256 * 1024 * 1024" in rootProject.file(
        ...
        listOf(
            ...
            rootProject.file("fabric-server/src/main/kotlin/axion/server/fabric/AxionFabricServerHistory.kt").absolutePath,
        ).forEach { historyPath ->
        ...
        listOf(
            "src/client/kotlin/axion/client/history/HistoryManager.kt",
            rootProject.file("paper-plugin/src/main/kotlin/axion/server/paper/AxionServerHistory.kt").absolutePath,
            rootProject.file("fabric-server/src/main/kotlin/axion/server/fabric/AxionFabricServerHistory.kt").absolutePath,
        ).forEach { historyPath ->
            val source = file(historyPath).readText()
...
tasks.jar {
    ...
    from(rootProject.layout.projectDirectory.dir("protocol/build/classes/kotlin/main"))
    if (supportsFabricDedicatedServer) {
        from(rootProject.layout.projectDirectory.dir("fabric-server/build/classes/kotlin/main"))
    }
...
tasks.register("setupPaperServer") {
    ...
    doLast {
        ...
        val pluginJar = rootProject.file("paper-plugin/build/libs/${rangeTag}/AxionPaper-v${modVersion}-${rangeTag}.jar")
        if (pluginJar.exists()) {
