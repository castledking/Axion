...
object VersionCompatImpl : VersionCompat {
    ...
    override fun stringToBlockState(str: String): BlockState? {
        val registryManager = getRegistryManager() ?: return null
        return runCatching {
            val wrapper = registryManager.getWrapperOrThrow(RegistryKeys.BLOCK)
            BlockArgumentParser.block(wrapper, str, false).blockState()
        }.getOrNull()
    }

    override fun itemStackToNbt(stack: ItemStack): NbtCompound {
        val ops = getRegistryOps() ?: return NbtCompound()
        return runCatching {
            ItemStack.CODEC.encodeStart(ops, stack).getOrThrow() as? NbtCompound ?: NbtCompound()
    ...
    fun applyBlockEntity(world: net.minecraft.world.World, write: BlockWrite, suppressUpdates: Boolean = true) {
        ...
        val blockEntity = net.minecraft.block.entity.BlockEntity.createFromNbt(
            write.pos,
            write.state,
            restored,
    ...
    fun getScaledMouseX(client: MinecraftClient): Double =
        client.mouse.x * client.window.scaledWidth / client.window.width

    fun getScaledMouseY(client: MinecraftClient): Double =
        client.mouse.y * client.window.scaledHeight / client.window.height
    ...
    /** Draws item count/durability overlay. In 1.21.0-1.21.1, DrawContext does
     *  not have drawStackOverlay - use drawItemInSlot instead. */
    ...
    // 1.21-1.21.1: drawTexture takes an Int (layer ID) as first arg, not a function reference
    ...
    fun drawGuiTextureRegion(
        ...
        context.drawTexture(texture, x, y, u.toFloat(), v.toFloat(), width, height, textureWidth, textureHeight)
    }
    ...
    override fun blockRenderManagerRenderBlock(
        manager: Any,
        state: BlockState,
        pos: Any,
        world: Any,
    ...
    override fun blockRenderManagerRenderFluid(
        manager: Any,
        pos: Any,
        world: Any,
        consumer: Any,
    ...
    // 1.21-1.21.1: loadEntityWithPassengers takes SpawnReason, not Function
    override fun entityTypeLoadEntityWithPassengers(
        tag: NbtCompound,
        world: Any,
        spawnReason: Any,
        entityProcessor: (Any) -> Any
    ): Any? =
        net.minecraft.entity.EntityType.loadEntityWithPassengers(tag, world as net.minecraft.world.World) { entity ->
            entityProcessor(entity) as Entity
        }
    ...
    override fun blockArgumentParserBlock(registry: Any, state: String): Any =
        BlockArgumentParser.block((registry as DynamicRegistryManager).getWrapperOrThrow(RegistryKeys.BLOCK), state, false)
