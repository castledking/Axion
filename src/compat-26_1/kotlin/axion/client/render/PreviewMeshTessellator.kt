...
object PreviewMeshTessellator {
    ...
    fun buildShellMesh(
        ...
        mergedFaces.asSequence().take(maxQuads).forEach { quads += it }
    ...
    private fun mergeFaces(faces: List<ResolvedFace>): List<PreviewQuad> {
        if (faces.isEmpty()) {
            return emptyList()
        }
    ...
    private fun faceGridCoords(blockPos: BlockPos, face: Direction): Pair<Int, Int> {
    ...
    private fun facePlane(blockPos: BlockPos, face: Direction): Int {
        return when (face) {
    ...
    private fun isFullCube(box: Box, blockPos: BlockPos): Boolean {
