...
object PreviewDirectionArrowRenderer {
    ...
    fun render(context: AxionWorldRenderContext, region: BlockRegion) {
        ...
        val v = axisDirection.vector
        render(context, region, Vec3i(v.x, v.y, v.z))
    }
    ...
    private fun liveLookDirection(): Direction? {
        ...
        val az = abs(look.z)
        return when {
            ax >= ay && ax >= az -> if (look.x >= 0) Direction.EAST else Direction.WEST
            ay >= ax && ay >= az -> if (look.y >= 0) Direction.UP else Direction.DOWN
            else -> if (look.z >= 0) Direction.SOUTH else Direction.NORTH
        }
    }
    ...
    private fun emitPrism(
        ...
        emitQuad(consumer, entry, cameraOffset, cornersStart[0], cornersStart[1], cornersEnd[1], cornersEnd[0], color)
        emitQuad(consumer, entry, cameraOffset, cornersStart[1], cornersStart[2], cornersEnd[2], cornersEnd[1], color)
        emitQuad(consumer, entry, cameraOffset, cornersStart[2], cornersStart[3], cornersEnd[3], cornersEnd[2], color)
        emitQuad(consumer, entry, cameraOffset, cornersStart[3], cornersStart[0], cornersEnd[0], cornersEnd[3], color)
        emitQuad(consumer, entry, cameraOffset, cornersStart[0], cornersStart[3], cornersStart[2], cornersStart[1], color)
    }
    ...
    private fun emitFrustum(
        ...
        emitQuad(consumer, entry, cameraOffset, base[0], base[1], tip[1], tip[0], color)
        emitQuad(consumer, entry, cameraOffset, base[1], base[2], tip[2], tip[1], color)
        emitQuad(consumer, entry, cameraOffset, base[2], base[3], tip[3], tip[2], color)
        emitQuad(consumer, entry, cameraOffset, base[3], base[0], tip[0], tip[3], color)
        emitQuad(consumer, entry, cameraOffset, tip[0], tip[1], tip[2], tip[3], color)
    }
    ...
    private fun emitQuad(
        ...
        emitVertex(consumer, entry, cameraOffset, a, color)
        emitVertex(consumer, entry, cameraOffset, b, color)
        emitVertex(consumer, entry, cameraOffset, c, color)
        emitVertex(consumer, entry, cameraOffset, d, color)
    }
    ...
    private fun emitVertex(
        ...
        consumer.vertex(
            entry,
            (point.x - cameraOffset.x).toFloat(),
            (point.y - cameraOffset.y).toFloat(),
            (point.z - cameraOffset.z).toFloat(),
