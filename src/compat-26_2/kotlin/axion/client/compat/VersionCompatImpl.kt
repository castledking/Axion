...
object VersionCompatImpl : VersionCompat {
    ...
    // 26.2 narrowed DynamicUniforms.writeTransform from the read-only joml
    // interfaces (Matrix4fc/Vector4fc/Vector3fc) back to the concrete types.
    fun writeDynamicUniforms(
        dynamicUniforms: net.minecraft.client.gl.DynamicUniforms,
        mvMatrix: org.joml.Matrix4f,
        colorTint: org.joml.Vector4f,
        zeroVec: org.joml.Vector3f,
        normalMatrix: org.joml.Matrix4f,
        lineWidth: Float,
    ): GpuBufferSlice {
    ...
    // 26.1 pulled the preview toward the camera with
    // GlStateManager._polygonOffset(-1.0f, -1.0f). 26.2 makes the render backend
    // switchable and defaults to Vulkan, so global GL state is no longer a
    // supported way to say this — the offset belongs to the pipeline. 26.2 also
    // adopted a reversed depth buffer, which flips the sign: vanilla's text
    // layering moved from GL (-1.0f, -10.0f) to DepthStencilState(..., 1.0f, 10.0f)
    // in the same update, and every vanilla depth test moved from
    // LESS_THAN_OR_EQUAL to GREATER_THAN_OR_EQUAL. A negative bias here would
    // push the preview *away* from the camera and z-fight against terrain.
    private const val PREVIEW_DEPTH_BIAS_SCALE = 1.0f
    private const val PREVIEW_DEPTH_BIAS_CONSTANT = 1.0f
    ...
    fun getPreviewShellPipeline(
        ...
        return previewShellPipelines.computeIfAbsent(drawMode) {
            ...
            // DynamicTransforms and Projection uniforms the preview shell shader
            // reads; SAMPLER0 carries its block atlas. Order matches vanilla's
            // own block pipelines, which bind the matrices group before samplers.
            RenderPipeline.builder()
                ...
                .withDepthStencilState(
                    DepthStencilState(
                        ...
                        false,
                        PREVIEW_DEPTH_BIAS_SCALE,
                        PREVIEW_DEPTH_BIAS_CONSTANT,
                    ),
                )
                .withCull(true)
    ...
    override fun worldSpawnNewEntityAndPassengers(world: Any, entity: Any): Boolean {
        // 26.2 API - addFreshEntityWithPassengers returns Unit
    ...
    // MinecraftClient API helpers for 26.2
    ...
    // Direction/BlockState API helpers for 26.2
    override fun directionGetVector(direction: Any): Any {
        // 26.2 API for direction vector is different
    ...
    // Registry/BlockArgumentParser API helpers for 26.2
    ...
    // ItemStack codec helper for 26.2.x
    ...
    override fun createAxionPluginPayloadCodec(): Any {
        // 26.2.x aliases PacketCodec to StreamCodec, whose factory is named of().
        ...
        val method = codecClass.methods.firstOrNull { it.name == "ofStatic" && it.parameterCount == 2 }
            ...
            ?: throw NoSuchMethodError("No compatible PacketCodec/StreamCodec factory method found in 26.2.x")
