...
class ChunkedPreviewSession(val previewId: String) : AutoCloseable {
    ...
    private fun buildGlobalBuffer(
        ...
        // staging/transient allocators now — and renamed mode() to
        // primitiveTopology(). BIG_BUFFER_SIZE is what the block layers sized
        // themselves to in 26.1.
        val allocator = BufferAllocator(net.minecraft.client.render.RenderLayer.BIG_BUFFER_SIZE)
        return try {
            val bufferBuilder = BufferBuilder(allocator, layer.primitiveTopology(), layer.format())
