...
class AxionFabricCommittedDiffBuilder(
    ...
    fun build(
        ...
        val before = touched.associateWith(::snapshot)
        apply()
        val changes = touched.mapNotNull { pos ->
    ...
    private fun snapshot(pos: IntVector3): BlockSnapshot {
        ...
        return BlockSnapshot(
            blockState = BlockArgumentParser.stringifyBlockState(world.getBlockState(blockPos)),
            blockEntityData = AxionFabricBlockEntitySnapshotService.capture(world, blockPos),
        )
    }
